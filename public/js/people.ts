...
import { Action, ActionType, ActionBar } from "./actionBar.ts";
import { overlayLayer } from "./game.ts";
...
class People implements Drawable {
    ...
    *getPeople(n = this.quantity) {
    ...
    click(event: GameEvent) {
        ...
        const actionBar = overlayLayer.find(element => element instanceof ActionBar) as ActionBar;
        if (actionBar) {
